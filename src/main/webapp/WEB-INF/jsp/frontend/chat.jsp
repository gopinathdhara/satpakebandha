<%@ page language="java" contentType="text/html; charset=ISO-8859-1"
    pageEncoding="ISO-8859-1"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%> 
<!DOCTYPE html>
<html dir="ltr" lang="en">

<%@ include file="includes/headcss.jsp" %>

<link href="<c:url value="/resources/chat/bootstrap.min.css" />" rel="stylesheet" id="bootstrap-css">
<link rel="stylesheet" href="https://use.fontawesome.com/releases/v5.5.0/css/all.css" integrity="sha384-B4dIYHKNBt8Bc12p+WXckhzcICo0wtJAoU8YZTY5qE0Id1GSseTk6S+L3BlXeVIU" crossorigin="anonymous">
<link rel="stylesheet" type="text/css" href="<c:url value="/resources/chat/jquery.mCustomScrollbar.min.css"/>">
<script type="text/javascript" src="<c:url value="/resources/chat/jquery.mCustomScrollbar.min.js"/>"></script>

<link rel="stylesheet" href="<c:url value="/resources/chat/chat.css?v=1.0" />" >
<script type="text/javascript" src="<c:url value="/resources/chat/chat.js" />"></script>


<body class="has-side-panel side-panel-right fullwidth-page side-push-panel">
<div class="body-overlay"></div>

<div id="wrapper" class="clearfix">
  <!-- preloader -->
  <%@ include file="includes/preloader.jsp" %>
  
  <%@ include file="includes/headerinner.jsp" %>
  
  <% 
   Long receiverid=(long) 0;
  try
  {
	  receiverid=Long.parseLong(request.getParameter("receiverid"));
  }
  catch(Exception e)
  {
	  receiverid=(long) 0;
  }
   %>
  
<input type="hidden" id="demousrimg" value="<c:url value="/resources/images/userimages/" />"/>
<input type="hidden" id="liveusrimg" value="<c:url value="/resources/userprofileimages/" />"/>
<input type="hidden" id="urlimg" value="<c:url value="/resources/images/" />"/>
<input type="hidden" id="receiverid" value="<%=receiverid%>"/>
<input type="hidden" id="sessionuid" name="sessionuid" value="${sessionuid }"/>

<c:set var="sessionuid" value="${sessionuid }" />
<c:set var="receiverid" value="<%=receiverid%>" />  
 <!--  <a class="scrollToTop" href="#"><i class="fa fa-angle-up"></i></a>-->

<!-- end wrapper -->
				<div class="container-fluid h-100">
				
				
							<div class="row justify-content-center h-100">
								<div class="col-md-4 col-xl-3 chat"><div class="card mb-sm-3 mb-md-0 contacts_card">
									<div class="card-header">
										<!--<div class="input-group">
												<input type="text" placeholder="Search..." name="" class="form-control search">
												<div class="input-group-prepend">
													<span class="input-group-text search_btn"><i class="fas fa-search"></i></span>
												</div>
											</div>-->
									</div>
									<div class="card-body contacts_body">
										<ui class="contacts">
										<c:set var="countuserstatus" value="0" /> 
										<c:forEach items="${onlineuser}" var="onlineuser">
										
						                	<c:set var="countuserstatus" value="1" />
						                	
						                	<li <c:if test = "${onlineuser.getId()==receiverid}"> class="active" </c:if> >
												<div class="d-flex bd-highlight">
													<div class="img_cont">
														<c:if test = "${onlineuser.getProfile_image()!=''}">
															<img src="<c:url value="/resources/userprofileimages/${onlineuser.getProfile_image()}" />" class="rounded-circle user_img">
														</c:if>
														
														<c:if test = "${onlineuser.getProfile_image()==''}">
														
																<c:if test = "${onlineuser.getGender()=='Female'}">
																	<img src="<c:url value="/resources/images/userimages/demouser3.png" />" class="rounded-circle user_img">
														
																</c:if>
																
																<c:if test = "${onlineuser.getGender()=='Male'}">
																	<img src="<c:url value="/resources/images/userimages/demouser2.jpg" />" class="rounded-circle user_img">
														
																</c:if>
														</c:if>
														<c:if test = "${onlineuser.getOnline_status()==1}">
														<span class="online_icon"></span>
														</c:if>
													</div>
													<div class="user_info">
														<span><a href="chat?receiverid=${onlineuser.getId()}" class="chatusrnam">${onlineuser.getName()}</a></span>
														<p class="chtmatmonyid">${onlineuser.getMatrimony_id()} </p>
													</div>
												</div>
											</li>
						                	
						                	
						                	<br/>
										
										</c:forEach>
										
										
									
										
										</ui>
									</div>
									<div class="card-footer"></div>
								</div></div>
								<div class="col-md-8 col-xl-6 chat">
								
								
									<div class="card">
									<% if(receiverid>0){ %>
										<div class="card-header msg_head">
											<div class="d-flex bd-highlight">
												<div class="img_cont">
													
													
														<c:if test = "${receiver_info.getProfile_image()!=''}">
															<img src="<c:url value="/resources/userprofileimages/${receiver_info.getProfile_image()}" />" class="rounded-circle user_img">
														</c:if>
														
														<c:if test = "${receiver_info.getProfile_image()==''}">
														
																<c:if test = "${receiver_info.getGender()=='Female'}">
																	<img src="<c:url value="/resources/images/userimages/demouser3.png" />" class="rounded-circle user_img">
														
																</c:if>
																
																<c:if test = "${receiver_info.getGender()=='Male'}">
																	<img src="<c:url value="/resources/images/userimages/demouser2.jpg" />" class="rounded-circle user_img">
														
																</c:if>
												    	</c:if>
													
														<c:if test = "${receiver_info.getOnline_status()==1}">
														<span class="online_icon"></span>
														</c:if>
													
												</div>
												<div class="user_info">
													<span> ${ receiver_info.getName() } </span>
													<!-- <p>1767 Messages</p> -->
												</div>
												<div class="video_cam">
													<!-- <span><i class="fas fa-video"></i></span> -->
													<!--  <span><i class="fas fa-phone"></i></span>-->
												</div>
											</div>
											<span id="action_menu_btn"><i class="fas fa-ellipsis-v"></i></span>
											<div class="action_menu">
												<ul>
													<li><a href="profiledetails?usermatrimonyid=${receiver_info.getMatrimony_id()}" target="_blank" class="chtproflnk"><i class="fas fa-user-circle"></i> View profile</a></li>
													<!--  <li><i class="fas fa-users"></i> Add to close friends</li>
													<li><i class="fas fa-plus"></i> Add to group</li>
													<li><i class="fas fa-ban"></i> Block</li>-->
												</ul>
											</div>
										</div>
										
										<div class="card-body msg_card_body" id="msgcntnr">
											
											<!-- <div class="d-flex justify-content-end mb-4">
												<div class="msg_cotainer_send">
													Ok, thank you have a good day
													<span class="msg_time_send">9:10 AM, Today</span>
												</div>
												<div class="img_cont_msg">
										<img src="data:image/jpeg;base64," class="rounded-circle user_img_msg">
												</div>
											</div>
											<div class="d-flex justify-content-start mb-4">
												<div class="img_cont_msg">
													<img src="https://static.turbosquid.com/Preview/001292/481/WV/_D.jpg" class="rounded-circle user_img_msg">
												</div>
												<div class="msg_cotainer">
													Bye, see you
													<span class="msg_time">9:12 AM, Today</span>
												</div>
											</div> -->
											
											
											<c:set var = "msgid"  value = "0"/>
											
											<c:forEach items="${message_info}" var="message_info">
										
											 <c:set var = "msgid"  value = "${message_info.getMessageid()}"/>
											
											<c:if test = "${message_info.getUserid()==sessionuid}">
											
											
												<div class="d-flex justify-content-start mb-4">
													<div class="img_cont_msg">
														
														
														<c:if test = "${message_info.getProfile_image()!=''}">
															<img src="<c:url value="/resources/userprofileimages/${message_info.getProfile_image()}" />" class="rounded-circle user_img_msg">
														</c:if>
														
														<c:if test = "${message_info.getProfile_image()==''}">
														
																<c:if test = "${message_info.getGender()=='Female'}">
																	<img src="<c:url value="/resources/images/userimages/demouser3.png" />" class="rounded-circle user_img_msg">
														
																</c:if>
																
																<c:if test = "${message_info.getGender()=='Male'}">
																	<img src="<c:url value="/resources/images/userimages/demouser2.jpg" />" class="rounded-circle user_img_msg">
														
																</c:if>
														</c:if>
														
														
													</div>
													<div class="msg_cotainer">${message_info.getMsg()}<span class="msg_time">${message_info.getCreated_date()}</span></div>
												</div>
											
											</c:if>
											
											<c:if test = "${message_info.getUserid()!=sessionuid}">
											
											<div class="d-flex justify-content-end mb-4">
												<div class="msg_cotainer_send">${message_info.getMsg()}<span class="msg_time_send">${message_info.getCreated_date()}</span></div>
												<div class="img_cont_msg">
														<c:if test = "${message_info.getProfile_image()!=''}">
															<img src="<c:url value="/resources/userprofileimages/${message_info.getProfile_image()}" />" class="rounded-circle user_img_msg">
														</c:if>
														
														<c:if test = "${message_info.getProfile_image()==''}">
														
																<c:if test = "${message_info.getGender()=='Female'}">
																	<img src="<c:url value="/resources/images/userimages/demouser3.png" />" class="rounded-circle user_img_msg">
														
																</c:if>
																
																<c:if test = "${message_info.getGender()=='Male'}">
																	<img src="<c:url value="/resources/images/userimages/demouser2.jpg" />" class="rounded-circle user_img_msg">
														
																</c:if>
														</c:if>
												</div>
											</div>
											
											</c:if>
											
											</c:forEach>
											
										</div>
										<div class="card-footer">
											<div class="input-group">
												<!-- <div class="input-group-append">
													<span class="input-group-text attach_btn"><i class="fas fa-paperclip"></i></span>
												</div> -->
												<textarea name="" class="form-control type_msg" placeholder="Type your message..." id="message_input"></textarea>
												<div class="input-group-append">
													<span class="input-group-text send_btn" id="sendbutton"> <i class="fas fa-location-arrow"></i></span>
												</div>
											</div>
										</div>
										<% } else { %>
												<c:if test = "${countuserstatus==0}">
													<h4 class="intchatmsg">There is no online members</h4>
												</c:if>
												<c:if test = "${countuserstatus==1}">
												<h4 class="intchatmsg">Please initiate chat with members</h4>
												</c:if>
										<% } %>
									</div>
									
									
									
								</div>
							</div>
				</div>
<!-- Footer Scripts -->
</div>
<!-- JS | Custom script for all pages -->
<%@ include file="includes/footerjs.jsp" %>

<audio id="myAudio" src="<c:url value="/resources/chat/goes-without-saying.mp3" />" muted autoplay ></audio>

<input type="hidden" id="lastmsgid" value="${msgid}"/>

<script>

$(document).ready(function(){
	
	
	$("#chatusrnam").click(function(){
			
		  
		  document.getElementById('myAudio').play();
		  document.getElementById('myAudio').muted = true;
		 
	     
	})
	
    var objDiv = document.getElementById("msgcntnr");
    objDiv.scrollTop = objDiv.scrollHeight;
    check_if_message_comes();
    
    
})   
var msgid=0; 

//check if new message comes    
function check_if_message_comes()
{
	
    var lastmsg_id=$("#lastmsgid").val();
    var receiverid=$("#receiverid").val();
    
    //console.log("hi");
        $.ajax({
                    type: "POST",
                    url: "check_if_message_comes",
                    data: "receiverid="+receiverid+"&lastmsgid="+lastmsg_id,  
                    async: false,
          		    dataType: 'html',
          		    cache: false,
                    beforeSend: function(){ },
                    success: function(html) 
                    {
                        
                            //console.log(html);
                            var response = JSON.parse(html);
                            var str="";
                            var findflag=0;
                              
                            $.each(response, function (key, val) {
                            	
                            	msgid=val.messageid;
                            	findflag=1;
                            	str+='<div class="d-flex justify-content-end mb-4"><div class="msg_cotainer_send">'+val.msg+'<span class="msg_time_send">'+val.created_date+'</span></div><div class="img_cont_msg">';
                            	
                            	if(val.gender=='Female')
      						  {
      							  
      							  if(val.profile_image=='')
      							  {
      				 	 			var demoimg=$("#demousrimg").val()+'demouser3.png';
      							  	
      							  }
      						  else
      							  {
      							  	var demoimg=$("#liveusrimg").val()+val.profile_image;
      							  }
      						 	
      						  }
      					  else
      						  {
      						 	 if(val.profile_image=='')
      							  {
      						 		var demoimg=$("#demousrimg").val()+'demouser2.jpg';
      							  	
      							  }
      						  else
      							  {
      							  	var demoimg=$("#liveusrimg").val()+val.profile_image;
      							  }
      						 	
      						  }
                            	
                            	str+='<img src="'+demoimg+'" class="rounded-circle user_img_msg"></div></div>';
          					  
          				  	})       
                               
                           
                            if(msgid!=0)
                                {
                                    $("#lastmsgid").val(msgid);
                                }
                            
                            $("#msgcntnr").append(str);
                         
                            if(str!="")
                            	{
                            	
                                	document.getElementById('myAudio').play();
                                	document.getElementById('myAudio').muted = false;
                            	}
                            if(findflag==1)
                            {
                                console.log(findflag);
                                objDiv = document.getElementById("msgcntnr");
                                objDiv.scrollTop = objDiv.scrollHeight;
                            }
                        
                          setTimeout(function(){
                              check_if_message_comes();
                          },3000) 
                    },

            });
}

var x=0;
$(document).on('click','#sendbutton',function(){
	
	if(x==0)
		{
		document.getElementById('myAudio').play();
		document.getElementById('myAudio').muted = true;
		}
	
	x++;
    var message_input=$("#message_input").val();
    var receiverid=$("#receiverid").val();
    
if(message_input!="")
 {
	
	var type='ajax';
	$.ajax({
		
		  url: "check_ifpremium_user_message",
		  cache: false,
		  type: "POST",
		  data: "type="+type,
		  async: false,
		  dataType: 'html',
		  //success callback
		  success: function(html){
			  
			  var response = JSON.parse(html);
			 
			  $.each(response, function (key, val) {
				  if(val.premiumflag==1)
					  {
					  
							  $.ajax({
				                    type: "POST",
				                    url: "insert_message",
				                    data: "receiverid="+receiverid+"&message_input="+message_input,
				                    async: false,
				          		    dataType: 'html',
				          		    cache: false,
				                    beforeSend: function(){ },
				                    success: function(html) 
				                    {
				                            $("#message_input").val('');
				                        
				                            
				                            var response = JSON.parse(html);
				                            var str="";
				                            var findflag=0;
				                            //console.log(response);
				                           if(jQuery.isEmptyObject(response)==false)
				                           {    
				                                   $.each(response, function (index, val) {
				                                	   
				                                	   msgid=val.messageid;
				                                	   
				                                   		findflag=1;
				                                   		
				                                   		if(val.gender=='Female')
				                						  {
				                							  
				                							  if(val.profile_image=='')
				                							  {
				                				 	 			var demoimg=$("#demousrimg").val()+'demouser3.png';
				                							  	
				                							  }
				                						  else
				                							  {
				                							  	var demoimg=$("#liveusrimg").val()+val.profile_image;
				                							  }
				                						 	
				                						  }
				                					  else
				                						  {
				                						 	 if(val.profile_image=='')
				                							  {
				                						 		var demoimg=$("#demousrimg").val()+'demouser2.jpg';
				                							  	
				                							  }
				                						  else
				                							  {
				                							  	var demoimg=$("#liveusrimg").val()+val.profile_image;
				                							  }
				                						 	
				                						  }
				                                     
				                                     str+='<div class="d-flex justify-content-start mb-4"><div class="img_cont_msg"><img src="'+demoimg+'" class="rounded-circle user_img_msg"></div><div class="msg_cotainer">'+val.msg+'<span class="msg_time">'+val.created_date+'</span></div></div>';
				                                     
				                                });
				                               
				                           }
				                            
				                            
				                            if(msgid!=0)
				                                {
				                                    $("#lastmsgid").val(msgid);
				                                }
				                            
				                            $("#msgcntnr").append(str);
				                            if(findflag==1)
				                            {
				                                //console.log(findflag);
				                                objDiv = document.getElementById("msgcntnr");
				                                objDiv.scrollTop = objDiv.scrollHeight;
				                            }
				                        
				                         
				                    },
		
				            });
					  }
				  else
					  {
					  	swal("", "Please take membership to chat with other members.", "error");
					  }
			  })
		  }
	});
	
	
    }
    else
    {
            swal(" ","Please enter your message", "warning");
    }
})   

</script>
</body>
</html>